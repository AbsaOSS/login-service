...
class TokenControllerTest extends AnyFlatSpec
  ...
  it should "return tokens generated by mocked JWTService for the basic-auth authenticated user" in {
    when(jwtService.generateAccessToken(FakeAuthentication.fakeUser, None)).thenReturn(fakeAccessJwt)
  ...
  it should "return tokens generated by mocked JWTService for the authenticated user with group-prefixes (single)(case irrelevant)" in {
    ...
    val expectedPrefixesConfig = PrefixesConfig(Set("first"), caseSensitive = false)
    when(jwtService.generateAccessToken(fakeUserFilteredGroups, Some(expectedPrefixesConfig))).thenReturn(fakeAccessJwt)
  ...
  it should "return tokens generated by mocked JWTService for the authenticated user with group-prefixes (multiple ,-separated) (case-sensitive)" in {
    ...
    val expectedUser = fakeUserWithGroups(userGroups) // passed as is, the JWTService does the filtering
    val expectedPrefixesConfig = PrefixesConfig(Set("second", "THIRD", "fOurth", "nonexistent"), caseSensitive = true)
    when(jwtService.generateAccessToken(expectedUser, Some(expectedPrefixesConfig))).thenReturn(fakeAccessJwt)
  ...
  Seq(
    ...
    it should s"return tokens generated by mocked JWTService for the authenticated user with group-prefixes (multiple ,-separated) ($caseName)" in {
      ...
      val expectedUser = fakeUserWithGroups(userGroups) // passed as is, the JWTService does the filtering
      val expectedPrefixesConfig = PrefixesConfig(Set("second", "THIRD", "fOurth", "nonexistent"), caseSensitive = false)
      when(jwtService.generateAccessToken(expectedUser, Some(expectedPrefixesConfig))).thenReturn(fakeAccessJwt)
      when(jwtService.generateRefreshToken(expectedUser)).thenReturn(fakeRefreshJwt)
      when(jwtService.getConfiguredRefreshExpDuration).thenReturn(refreshDuration)
  ...
  it should "fail for anonymous (not authenticated) user" in {
    when(jwtService.generateAccessToken(any[User], any[Option[PrefixesConfig]], any[Boolean])).thenReturn(fakeAccessJwt)
