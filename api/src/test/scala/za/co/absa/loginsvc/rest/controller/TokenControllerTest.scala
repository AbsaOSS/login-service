...
import za.co.absa.loginsvc.rest.config.provider.{ConfigProvider, ExperimentalRestConfigProvider}
...
class TokenControllerTest extends AnyFlatSpec
  ...
  it should "return tokens generated by mocked JWTService for the kerberos authenticated user" in {
    ...
    assertExpectedResponseFields(
      ...
      expectedJsonBody = s"""{"token": "${fakeAccessJwt.token}", "refresh": "${fakeRefreshJwt.token}"}"""
    )(Some(FakeAuthentication.fakeUserDetailsAuthentication))
  }
  ...
  it should "return tokens generated by mocked JWTService for the basic-auth authenticated user" in {
    ...
    assertExpectedResponseFields(
      "/token/experimental/get-generate",
      Get()
    )(
      expectedJsonBody = s"""{"token": "${fakeAccessJwt.token}", "refresh": "${fakeRefreshJwt.token}"}"""
    )(Some(FakeAuthentication.fakeUserAuthentication))
