...
class TokenControllerTest extends AnyFlatSpec with ControllerIntegrationTestBase {
  ...
  val fakeAccessJwt = "abc.fakeJWTToken.abc"
  val fakeRefreshJwt = "ab.fakeJWTToken.cd"
  ...
  it should "return tokens generated by mocked JWTService for the authenticated user" in {
    ...
    assertOkAndResultBodyJsonEquals(
      "/token/generate",
      Post(),
      s"""{"token": "$fakeAccessJwt", "refresh": "$fakeRefreshJwt"}"""
  ...
  it should "return refresh tokens by mocked JWTService for the authenticated user" in {
    val newFakeAccessJwt = "abc.newFakeJWTToken.abc"

    when(jwtService.refreshToken(Tokens(fakeAccessJwt, fakeRefreshJwt))).thenReturn(newFakeAccessJwt)
